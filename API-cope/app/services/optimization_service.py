...
def optimize_fraud_detection_response(paths, nodes, edges, gnn_info):
    ...
    """
    try:
        if not paths or not nodes or not edges:
            logger.error("Missing required data for optimization")
            return {
                'error': '缺少必要的数据进行优化'
            }
        ...
        # Group low risk nodes by category
        ...
        for category, node_list in category_groups.items():
            if len(node_list) > 5:  # 只聚合数量足够多的节点
                try:
                    ...
                    logger.error(f"Error aggregating nodes for category {category}: {e}")
                    continue
            else:
                aggregated_nodes.extend(node_list)

        # Add high risk and connected nodes
        for node in nodes:
        ...
        for edge_data in edge_map.values():
            try:
                ...
                logger.error(f"Error adding aggregated edge: {e}")
                continue
        ...
        if gnn_info:
            try:
                ...
                logger.error(f"Error optimizing GNN info: {e}")
                optimized_gnn_info = {}

        logger.info("Successfully optimized fraud detection response")
        return {
        ...
        logger.error(f"Error optimizing fraud detection response: {e}")
        import traceback
        logger.error(traceback.format_exc())
        return {'error': f'优化诈骗检测响应数据失败: {str(e)}'} 